...
int gClientPort;
char gName[256]; //Nom d'un joueur
char gNames[4][256]; //Noms de tout les joueurs 
int gId;
int joueurSel;
int objetSel;
int guiltSel;
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				if ((mx<200) && (my<50) && (connectEnabled==1))
				{
					...
					sendMessageToServer(gClientIpAddress,gClientPort,gName);
		...
		switch (gbuffer[0])
		{
			...
			case 'V':
				/** RAJOUTER DU CODE ICI**/
				int j,  carte,  nbr;
				sscanf(gbuffer, "V %d %d %d", &j, &carte, &nbr);
				if(nbr != 100 && tableCartes[j][carte] == -1){ //si la valeur de la carte est la valeur par défault 
					tableCartes[j][carte] = nbr;
				}
